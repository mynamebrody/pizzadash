...
rl.on(
    ...
    function(chunk) {
        ...
        if (chunk !== null) {
            ...
            //Find Button
            if(data.indexOf('find button')>-1){
                return;
            }
            //Set Order (name)
            if(data.indexOf('set order ')>-1){
                return;
            }
            //Set button mac_address ordername
            if(data.indexOf('set button ')>-1){
                return;
            }
    ...
    if(!order.Payments[0].Number){
        rl.question(
            ...
            function(data){
                ...
                order.Payments[0].CardType = order.validateCC(data);
                placeOrder();
            }
        );
        return;
